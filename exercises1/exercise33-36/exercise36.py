...
def birthcounter(Djson,n=2): # n=1: yyyy to count, n=2: mm to count, n=3: dd to count
...
def birthplot(Djson):   # get the plot of couter nunber 
    Mon={1:'Jan', 2:'Feb', 3:'Mar', 4:'Apr', 5:'May', 6:'Jun', 7:'Jul', 8:'Aug', 9:'Sep', 10:'Oct', 11:'Nov', 12:'Dec'}
    x=[];y=[]
    for k, v in Djson.items():
        x.append([key for key, val in Mon.items() if k==val][0])
        y.append(int(v))
    plt.bar(x,y)
    plt.xticks(x,Djson.keys())
    plt.show()
...
if __name__=='__main__':
    ...
    while True:
        job=input('What do you want to do?(Input: I; Search: S; Counte: C; Plot: P or  end: E): ')
        if job=='I':
